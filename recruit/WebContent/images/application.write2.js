jQuery(function($){
	...
	*/
	...
	function change_step(step)
	{
		...
		$processbar.children("div.over").animate({"left":(step-1)*130, "background-position-x": -(step-1)*130});
	} // end function
	...
	$processbar			= $application.children("div.processbar");
	...
	$step2
		...
		.children("table.dynamic_data")
			.each(function(){
			var $this_table	= $(this);
			var $sample_tr	= $this_table.find("> tbody > tr:first").hide();
			...
			$this_table
				...
				.find("button.item_del")
					.live("click", function(){
						$(this).parents("tr.row:first").remove();

						if($this_table.find("> tbody > tr").size() <= 1)
							$this_table.trigger("item_add");
					})
					.end();
			})
							...
							.find("ul.data")
								...
								.find("> li > a")
									.click(function(){
										...
										$(popup_option["form"])
											.find("input")
												.filter("[name='"+popup_option["zip1"]+"']").val(zipcode[0]).end()
												.filter("[name='"+popup_option["zip2"]+"']").val(zipcode[1]).end()
							...
							.find("ul.data")
								...
								.find("> li > a")
									.click(function(){
										...
										$(popup_option["rel_hidden_input"]).val(code);
										$(popup_option["rel_text_input"]).val(code_name);
